...
- Length -> IP header field

- Total Length -> Packet Length

- Fragment Offset -> Instruccions per a tornar a montar un paquet molt gran el qual s’ha dividit en paquets petits.
...
##### DETECTION (Finding Irregularities in Fragment Offset)

En un anàlisi de tràfic de red hi han dos indicadors que hem d’estar alerta:

1.      Diverses peticions ICMP entre dos hosts, les quals son indicatives d’un scan amb nmap.

2.      Un munt de paquets fragmentats desde el mateix host.

### C.2) IP Source and Destination attacks

Hi han molts casos on podem veure tràfic de red irregular per paquets IPv4 i IPv6. Ho detectem amb els source and destination IP fields i considerant:
...
### C.3 IP Time-to-live Attacks

Elsa tacs TTL s’utilitzen principalment com a mitjà d’evasió per part dels atacants. Aquests estableixen intencionadament un TTL molt baix als seus paquets IP per intentar evadir els firewalls, IDS i els sistemes IPS.

##### DETECTION (Irregularities in IP TTL)

Detectar-ho en petites quantitas pot ser difícil, però afortunadament per a nosaltres, els atacants la majoria de vegades utilitzaran la manipulació del TTL per a fer escaneig de ports. Podem implementar un control que filtra o descarta els paquets que no tenen un TTL prou alt.

### C.4 Handshake Abnormalities TCP
...
-> JUST TOO MANY FLAGSÇ

### C.5 TCP Connection Resets and Hijacking
...
3. The attacker will specify the destination port to be thte same as one currently in use by one of our machines.

##### TCP Connection Hijacking
-> L'atacant prediu o adivina els números de sequència dels paquets TCP per fer-se passar per una de les parts que es comuniquen i inserir les seves pròpies dades en la sessió en curs.
...
##### DETECTION (ICMP Tunneling)

-> We can find it by looking at the contents of data per request and reply. Normalment els paquets ICMP son d'uns 48 bytes, per tant, si detectem paquets ICMP molt grans seran sospitosos.

##### PREVENTION

1. Bloquejar les peticions ICMP
2. Inspeccionar les Requests and Replies d'ICMP en busca de data rellevant.
